...
public class ManufactureDriver implements Driver<Manufacture>{
    ...
    public boolean update(Manufacture manufacture, long id) {
        session = sessionFactory.openSession();
        Transaction transaction = session.beginTransaction();
        Query q = session.createQuery("update Manufacture set card_number=:n, code=:a where ID = :id")
                .setParameter("id", id)
                .setParameter("a", manufacture.getCard_number())
                .setParameter("n", manufacture.getCode());
